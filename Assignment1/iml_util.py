...
def load_csv(test_subject, folder, filename):
    ...
    if test_subject == "test_subject_0":
        file_path = os.path.join(
            os.path.abspath(f"data/test_subjects/{test_subject}/gaze/" + folder),
            f"{filename}.csv",
        )
    else:
        file_path = os.path.join(
            os.path.abspath(f"data/output/{test_subject}/" + folder), f"{filename}.csv"
        )
...
def image_and_scatter(image, data_points):
    ...
    """
    fig, axes = plt.subplots(
        1, 3, figsize=(20, 6), gridspec_kw={"width_ratios": [2, 1, 1]}
    )
...
def create_image_grid(
    ...
    for ax, (image_file, img), center in zip(axs, images, centers):
        ...
        if center_shifted:
            ...
            cv.circle(
                img_color,
                (int(center_shifted[0]), int(center_shifted[1])),
                5,
                (0, 0, 255),
                -1,
            )
...
def gen_data_subject_0():
    ...
    for f in range(4):
        pupils_n = create_pupil_data(
            load_csv("test_subject_0", str(f), "normalized_pupils")
        )
        pupils = create_pupil_data(load_csv("test_subject_0", str(f), "pupils"))
        positions_n = create_position_data(
            load_csv("test_subject_0", str(f), "normalized_screen_coordinates")
        )
        positions = create_position_data(
            load_csv("test_subject_0", str(f), "positions")
        )
        images = [
            open_img(os.path.abspath(f"data/test_subjects/test_subject_0/gaze/{f}"), i)
...
def gen_data_subject(test_subject):
    ...
    for f in p:
        D.append(
            {
                f"pupils_train": create_pupil_data(
                    load_csv(test_subject, "grid", "mean_pupil_coordinates")
                ),
                f"pupils_test": create_pupil_data(
                    load_csv(test_subject, f, "mean_pupil_coordinates")
                ),
                f"screen_coordinates_train": create_position_data_collection(
                    load_csv(test_subject, "grid", "screen_coordinates")
                ),
                f"screen_coordinates_test": create_position_data_collection(
                    load_csv(test_subject, f, "screen_coordinates")
                ),
            }
        )
    return D
...
def plot_least_squares_plane(ax, results_dict, dataset, coor):
    ...
    x_grid, y_grid = np.meshgrid(
        np.linspace(min(x_train), max(x_train), 20),
        np.linspace(min(y_train), max(y_train), 20),
    )
    if coor == "y":
...
def plot_error_surfaces(a_fixed, b_fixed, c_fixed, x, y, z):
    ...
    for i in range(a_grid.shape[0]):
        for j in range(a_grid.shape[1]):
            error_grid_ab[i, j] = compute_error(
                a_grid[i, j], b_grid[i, j], c_fixed, x, y, z
            )
    ...
    surf = ax1.plot_surface(
        a_grid, b_grid, error_grid_ab, cmap="coolwarm", edgecolor="none"
    )
    ax1.set_xlabel("a")
    ax1.set_ylabel("b")
...
def plot_results_grid(results, grid_size):
    ...
    for i in range(grid_size):
        ...
        ax_0.scatter(
            predicted[:, 0], predicted[:, 1], label="Prediction", color=colors[0]
        )
        ax_0.scatter(
            ground_truth[:, 0],
            ground_truth[:, 1],
